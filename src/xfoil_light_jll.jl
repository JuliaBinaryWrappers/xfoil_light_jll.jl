...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import xfoil_light_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("xfoil_light_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "xfoil_light_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"xfoil_light", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "xfoil_light_jll", "src", "xfoil_light_jll.jl"))
        @info("xfoil_light_ll dev'ed out to /depot/dev/xfoil_light_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("70cc596b-f351-5640-b155-76ddf0ff62ca"))
platforms = [Pkg.Artifacts.unpack_platform(e, "xfoil_light", artifacts_toml) for e in artifacts["xfoil_light"]]
...
if best_platform === nothing
    @debug("Unable to load xfoil_light; unsupported platform $(triplet(platform_key_abi()))")
else
